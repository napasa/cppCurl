...
namespace Http
{
	...
	NETWORK_API const std::string & Request::Url() const
	{
		return url;
	}


	NETWORK_API void Request::Url(const std::string &val)
	{
		url = val;
	}


	NETWORK_API void Request::Unhandled(bool)
	{
		unhandled = false;
	}


	NETWORK_API bool Request::Unhandled() const
	{
		return unhandled;
	}


	NETWORK_API void * Request::UserData() const
	{
		return userData;
	}


	NETWORK_API void Request::UserData(void * val)
	{
		userData = val;
	}


	NETWORK_API std::vector<Http::UploadedData> & Request::Uploadeddatas()
	{
		return uploadeddatas;
	}


	NETWORK_API Http::Request::TYPE Request::Type() const
	{
#ifdef _DEBUG
	...
	NETWORK_API void Request::Type(TYPE val)
	{
		type = val; 
#ifdef _DEBUG
		assert(type == GET || type == POST);
	...
	Task::Task(const std::string &url, Http::Action *action, void * userdata /*= nullptr*/) :Request(url, userdata), Response(), action(action), mark(Task::markCouter++)
	{

	}

	Task::Task(const std::string &url, const std::vector<UploadedData> &uploadData, Http::Action *action, void *userData /*= nullptr*/) : Request(url, uploadData, userData), action(action), mark(Task::markCouter++)
	{
	...
	NETWORK_API UploadedData & UploadedData::operator=(const UploadedData &uploadData)
	{
		field = uploadData.Field(); key = uploadData.Key(), value = uploadData.Value();
		return *this;
	}

	NETWORK_API bool UploadedData::operator==(const UploadedData &postData) const
	{
		return field == postData.field && key == postData.key&&value == postData.value;
	}

	NETWORK_API UploadedData::FIELD UploadedData::Field() const
	{
		return field;
	}

	NETWORK_API	void UploadedData::Field(FIELD val)
	{
		field = val;
	}


	NETWORK_API const std::string & UploadedData::Key() const
	{
		return key;
	}


	NETWORK_API void UploadedData::Key(const std::string &val)
	{
		key = val;
	}


	NETWORK_API const std::string & UploadedData::Value() const
	{
		return value;
	}


	NETWORK_API void UploadedData::Value(const std::string &val)
	{
		value = val;
	}
